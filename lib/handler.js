      ...
      if (await fs.statSync(path.join(folder, v)).isDirectory()) {
        ...
        try {
          if (v.endsWith(".mjs")) {
            ...
            ((await import("file://" + path.join(process.cwd(), folder, v))).default)(cmd(path.join(process.cwd(), folder, v)));
          } else if (v.endsWith(".cjs") || v.endsWith(".js")) {
            // Untuk plugin CommonJS
            await require(path.join(process.cwd(), folder, v))(cmd(path.join(process.cwd(), folder, v)));
